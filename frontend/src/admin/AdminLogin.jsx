...
function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:4001/api/v1/admin/login', formData);
      console.log("Login Success:", response.data);
      toast.success("Login Success");
localStorage.removeItem("admin"); // optional cleanup
localStorage.setItem("admin", response.data.token); // no stringify!
    ...
    <div className="bg-gradient-to-r from-black to-blue-950 min-h-screen text-white flex flex-col">
      ...
      <div className="flex-grow flex items-center justify-center px-4">
        <div className="bg-gray-900 rounded-lg p-8 w-full max-w-md shadow-lg">
          ...
          <p className="text-gray-400 mb-6 text-center">Login to access Admin Dashboard</p>
